...
export class Cars {
        ...
        carsList: HTMLElement = document.createElement('ul');
        title: HTMLElement = document.createElement('h1');

        async getCars() {
            const carResponse = await fetch('http://localhost:3000/garage?_page=1&limit=7');
            const data = await carResponse.json();
            return data
        }

        getCar(data: [{name: string, color: string}]) {
            let output = '';
            data.forEach((car) => {
                output += `
                <li><div class='car-item'>
